...
function expediente (options) {
  ...
  // start to moment obj
  options.start = moment(options.start, defaultFormat);
  ...
  if (typeof options.hours === 'string') {
    ...
    duration = duration.bind(moment, defaultFormat);
  }
  ...
  result.remaining = format(moment(remainingFn('hours') + ':' + remainingFn('minutes') % 60, defaultFormat));
